...
func New(matches []Match, localNets *netaddr.IPSet, logIPs *netaddr.IPSet, shareStateWith *Filter, logf logger.Logf) *Filter {
	...
	f := &Filter{
		...
		cap4:     capMatchesFamily(matches, netaddr.IP.Is4),
		cap6:     capMatchesFamily(matches, netaddr.IP.Is6),
		local:    localNets,
		logIPs:   logIPs,
		state:    state,
	}
...
// capMatchesFamily returns the subset of ms for which keep(srcNet.IP)
// and the match is a capability grant.
func capMatchesFamily(ms matches, keep func(netaddr.IP) bool) matches {
	var ret matches
	for _, m := range ms {
		if len(m.Caps) == 0 {
			continue
...
func (f *Filter) AppendCaps(base []string, srcIP, dstIP netaddr.IP) []string {
	...
	for _, m := range mm {
		...
		for _, cm := range m.Caps {
			if cm.Cap != "" && cm.Net.Contains(dstIP) {
